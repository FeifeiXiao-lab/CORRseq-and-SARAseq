...
#' @export
modifiedSaRa <-function(Y, alpha = 0.01, h1 = 5, h2 = 10, h3 = 15, L =100, sigma = NULL, precise=10000, FINV=FINV, type=type) {
  ...
  if (TRUE %in% (length(cp.s)==0)) {
    ...
    Gaus.cluster = CNVcluster_new(Y, cp = cp.BIC, L = L, type=type)
